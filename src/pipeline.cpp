...
namespace vt
{
    ...
    VtPipeline::~VtPipeline()
    {
        ...
        vkDestroyPipeline(vtDevice.device(), graphicPipeline, nullptr);
    }
        ...
        const std::string &vertFilepath, const std::string &fragFilepath, const PipelineConfigInfo &configInfo)
    {
        ...
        if (vkCreateGraphicsPipelines(vtDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicPipeline) != VK_SUCCESS)
        {
